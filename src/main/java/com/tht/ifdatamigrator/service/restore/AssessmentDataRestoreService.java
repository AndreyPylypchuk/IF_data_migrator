...
public class AssessmentDataRestoreService {
    ...
    public void restore(AssessmentDTO a) {
        ...
        if (isNull(assId)) {
            ...
            createDefaultAssTranslate(translateId, a.getName());
        }

        for (Integer qId : a.getQualities()) {
            Map<String, Object> qParam = new HashMap<>();
            qParam.put("assessment_id", assId);
            qParam.put("quality_id", qId);
    ...
    private void createDefaultAssTranslate(long translateId, String name) {
        Map<String, String> translate = new HashMap<>();
        translate.put("assessment_desc", null);
        translate.put("assessment_name", name);
        translate.put("candidate_desc", null);
        translate.put("general_instructions", null);
        translate.put("thank_you_desc", null);

        service.createAssTranslation(translateId, translate, "en");
        translate.put("assessment_name", null);
        service.createAssTranslation(translateId, translate, "fr");
        service.createAssTranslation(translateId, translate, "es");
    }
}
